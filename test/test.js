...
describe("Sav3", function () {
  ...
  it("should disallow to exceed the max mint count for presale", async function () {
    ...
    const leaves = preSaleWhitelisted.map((account) =>
    ...
    await expect(
      sav3
        .connect(preSaleWhitelisted[1])
        .preSaleMint(
          6,
          tree.getHexProof(keccak256(preSaleWhitelisted[1].address)),
          {
  ...
  it("should disallow to exceed the max mint count for whitelist", async function () {
    ...
    await expect(
      sav3
        ...
        .whitelistMint(6, tree.getHexProof(keccak256(whitelisted[1].address)), {
          value: ethers.utils.parseUnits("25000000", "wei"),
        })
    ...
    await expect(
      sav3
        .connect(whitelisted[1])
        .whitelistMint(3, tree.getHexProof(keccak256(whitelisted[1].address)), {
          value: ethers.utils.parseUnits("25000000", "wei"),
        })
    ).not.to.be.reverted;
  ...
  it("should work with presale", async function () {
    ...
    await expect(
      sav3
        .connect(notPreSaleWhitelisted[1])
        .preSaleMint(
          2,
          tree.getHexProof(keccak256(notPreSaleWhitelisted[1].address)),
          {
            value: ethers.utils.parseUnits("70000", "wei"),
          }
        )
  ...
  it("should work with whitelist sale", async function () {
    ...
    await expect(
      sav3
        .connect(whitelisted[1])
        .whitelistMint(2, tree.getHexProof(keccak256(whitelisted[1].address)), {
    ...
    await expect(
      sav3
        .connect(notWhitelisted[1])
        .whitelistMint(
          2,
          tree.getHexProof(keccak256(notWhitelisted[1].address)),
          {
            value: ethers.utils.parseUnits("110000", "wei"),
          }
        )
  ...
  it("can't minted greater than total supply", async function () {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      await expect(
        sav3.connect(wallet).publicMint(50, {
          value: ethers.utils.parseUnits("25000000", "wei"),
        })
      ).not.to.be.reverted;
    }
  ...
  describe("reserved", async () => {
    it("#1", async () => {
      ...
      await expect(
        sav3.reservedMint(4, accounts[2].address)
      ).to.be.revertedWith("not enough reserved quantity");
      await expect(sav3.reservedMint(2, accounts[2].address)).not.to.be
        .reverted;
      await expect(
        sav3.reservedMint(2, accounts[2].address)
      ).to.be.revertedWith("not enough reserved quantity");
      await expect(sav3.reservedMint(1, accounts[2].address)).not.to.be
        .reverted;

      expect(await sav3.usedReservedQuantity()).to.be.equal(3);
      await expect(sav3.setReservedQuantity(2)).to.be.revertedWith(
        "reserved quantity is greater than used quantity"
      );
      expect(await sav3.usedReservedQuantity()).to.be.equal(3);
      await expect(sav3.setReservedQuantity(5)).not.to.be.reverted;
      expect(await sav3.usedReservedQuantity()).to.be.equal(3);

      await expect(sav3.setReservedQuantity(5001)).to.be.revertedWith(
        "reached max supply"
      );
    });
  });
  ...
  it("#2", async () => {
    ...
    await expect(
      sav3.connect(accounts[1]).publicMint(2, {
        value: ethers.utils.parseUnits("9000000000000000000", "wei"),
      })
    ).to.be.revertedWith("reached max supply");

    await expect(
      sav3.connect(accounts[1]).publicMint(1, {
        value: ethers.utils.parseUnits("9000000000000000000", "wei"),
      })
    ).not.to.be.reverted;
    ...
    await expect(
      sav3.connect(accounts[1]).publicMint(50, {
        value: ethers.utils.parseUnits("9000000000000000000", "wei"),
      })
    ).to.be.revertedWith("reached max supply");
    ...
    await expect(
      sav3.connect(accounts[1]).publicMint(49, {
        value: ethers.utils.parseUnits("9000000000000000000", "wei"),
      })
    ).not.to.be.reverted;
  });
...
describe("full scenario", () => {
  it("#1", async () => {
    ...
    const preSaleLeaves = preSaleWhitelisted.map((account) =>
      keccak256(account.address)
    );
    const preSaleTree = new MerkleTree(preSaleLeaves, keccak256, {
      sort: true,
    });
    const preSaleMerkleRoot = preSaleTree.getHexRoot();
    await sav3.setPreSaleMerkleRoot(preSaleMerkleRoot);

    const whitelistedLeaves = whitelisted.map((account) =>
      keccak256(account.address)
    );
    const whitelistedTree = new MerkleTree(whitelistedLeaves, keccak256, {
      sort: true,
    });
    ...
    await expect(
      sav3
        ...
        .preSaleMint(
          2,
          preSaleTree.getHexProof(keccak256(preSaleWhitelisted[1].address)),
          {
    ...
    await expect(
      sav3
        ...
        .whitelistMint(
          2,
          whitelistedTree.getHexProof(keccak256(whitelisted[1].address)),
          {
            value: ethers.utils.parseUnits("9000000000000000000", "wei"),
          }
        )
    ...
    await expect(
      sav3.connect(notWhitelisted[1]).publicMint(2, {
